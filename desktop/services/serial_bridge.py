"""
Serial Bridge Service — ESP32/LilyGo T-CAN485 USB Serial Communication
Windows-compatible version with COM port support.

Reads JSON data from ESP32 over USB serial, sends commands back.
Supports:
  - Direct USB-to-RS485 adapters
  - LilyGo T-CAN485 ESP32 (built-in CAN + RS485)
  - ESP32-S3 with USB-CDC or CP2102/CH340

Protocol: Newline-delimited JSON, 115200 baud (configurable)
"""
...
class SerialBridge(QObject):
    """Manages serial communication with ESP32/LilyGo T-CAN485 over USB"""
    ...
    def __init__(self, port='COM3', baud=115200, parent=None):
    ...
    def _connect(self, pyserial):
        ...
        try:
            self._serial = pyserial.Serial(
                ...
                self._baud,
                timeout=1,
                # Windows-specific: ensure proper DTR/RTS for ESP32 reset
                dsrdtr=False,
                rtscts=False
            )
